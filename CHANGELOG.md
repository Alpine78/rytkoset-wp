...
### Added
- `docker-compose.yml` / `docs/local-dev-wsl.md` / `CLAUDE.md`: paikallisen kehityksen voi nyt ajaa Windowsilla ilman Docker Desktopia (WSL2 + Docker Engine). Lisätty pyydettäessä käynnistyvä `wpcli`-palvelu (`wordpress:cli-php8.3`, `cli`-profiili) ja nimetty `wp_core`-volyymi WordPressin coren säilytykseen, jolloin asennuksen, suomenkielisen lokalisoinnin, teeman aktivoinnin ja osoiterakenteen saa pystyyn WP-CLI:llä ilman selainvelhoa. Uusi `docs/local-dev-wsl.md` kuvaa Docker Enginen asennuksen WSL:ään, repon sijoittamisen Linux-tiedostojärjestelmään, WP-CLI-bootstrapin ja plugin-huomiot.
