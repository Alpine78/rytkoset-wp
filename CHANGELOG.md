...
### Changed
- `.github/workflows/deploy-dev.yml`: kolmannen osapuolen GitHub Actions pinnattu muuttuvista versiotunnisteista tarkistettuihin commit-SHA:hin (supply chain -kovennus, finding 5). `actions/checkout@v5` → `@93cb6efe…` ja `SamKirkland/FTP-Deploy-Action@v4.4.0` → `@110f9186…`, versio säilytetty kommenttina Dependabotia varten. Muuttuva tagi voi osoittaa eri (mahdollisesti haitalliseen) commitiin myöhemmin, mikä on riski FTPS-credentiaaleja käsittelevässä deploy-workflow'ssa (#349).
