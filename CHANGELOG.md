...
### Changed
- `CLAUDE.md`: päivitetty vastaamaan nykyistä koodia (ei koodimuutoksia). `functions.php`-rivimäärä ~580 → ~880 ja kuvaukseen lisätty ostoskori-linkin (`rytkoset_theme_get_cart_link_markup`) ja bbPress-esityshelpperit; moduulitaulukkoon ja template-hierarkiaan lisätty `inc/coming-soon.php` ja `coming-soon.php` (WooCommercen "Tulossa pian" -block-templaatin ohitus); tapahtuma-CPT:n otsikko korjattu `event` → `rytkoset_event` (#342).
- `.github/workflows/deploy-dev.yml` ja `deploy-production.yml`: kaikki GitHub Actions pinnattu muuttuvista versiotunnisteista tarkistettuihin commit-SHA:hin (supply chain -kovennus, finding 5). `actions/checkout@v5` → `@93cb6efe…`, `SamKirkland/FTP-Deploy-Action@v4.4.0` → `@110f9186…` ja (tuotannossa) `shivammathur/setup-php@v2` → `@f3e473d1…`; versio säilytetty kommenttina Dependabotia varten. Muuttuva tagi voi osoittaa eri (mahdollisesti haitalliseen) commitiin myöhemmin, mikä on riski FTPS-credentiaaleja käsittelevissä deploy-workflow'issa (#349).
