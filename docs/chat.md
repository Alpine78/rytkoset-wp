...
- Hallitus-, puheenjohtaja- ja yksiselitteisesti yhdelle julkiselle sivulle osuvat henkilökysymykset saavat ennen Mistralia palvelimen varmentaman lähdeotteen. Hallitussivu ratkaistaan täsmällisellä polulla `sukuseura/sukuseuran-hallitus`; muut henkilöt haetaan enintään 60 julkaistusta sivusta kysymyksen erisnimillä. Sama julkaisu-, salasana- ja jäsenrajaus koskee hakua. Vain yksi paras sivuosuma hyväksytään, ote rajataan 3000 merkkiin ja Mistralille tehdään yksi tavallinen tekstikutsu asetuksella `tool_choice: "none"`, joten työkalumäärittely säilyy payloadissa mutta ylimääräistä työkalukierrosta ei voi syntyä. Puuttuva tai moniselitteinen osuma ei ohita `tool_choice: "any"` -invarianttia.
...
2. Hallitus-, puheenjohtaja- ja yksiselitteiset henkilökysymykset ratkaistaan ensin yllä kuvatulla palvelinhaulla. Kun varmennettu julkinen sivu löytyy, sen rajattu ote annetaan lähteenä yhdelle tavalliselle Mistral-kutsulle eikä epävakaata function calling -alkukierrosta tarvita. Muut henkilönimeä, harvinaista nimimuotoa, tekijyyttä, sääntöjen täsmällistä käsitettä tai ellipsimäistä/pronominilla tehtyä seurantakysymystä koskevat kysymykset käyttävät edelleen ensimmäisellä API-kierroksella `tool_choice: "any"` -pakkoa (#507). Tällaisia sääntökäsitteitä ovat esimerkiksi `toimintakausi`, `tilintarkastus` ja `nimenkirjoitus`; myös `Entä …?` pakottaa tuoreen sivunluvun. Pakotus poistetaan heti ensimmäisen työkalutuloksen jälkeen; tavalliset tukikysymykset säilyttävät Mistralin automaattisen työkaluvalinnan. System-prompt kieltää korvaamasta kysyttyä käsitettä samankaltaisella: hallituskausi, toimintakausi ja tilikausi käsitellään eri asioina. Kun Mistral vastaa `tool_calls`-rakenteella, backend suorittaa työkalun (`rytkoset_theme_chat_run_page_tool()`), liittää keskusteluun assistentin tool-call-viestin + `role: "tool"` -viestin (`tool_call_id`) ja tekee jatkokutsun. Kierroksia sallitaan oletuksena **2**. Viimeisellä sallitulla kierroksella `tool_choice: "none"` pakottaa tekstivastauksen.
...
**#604:n poikkeukset:** suora tilikausikysymys (myös taivutus- ja `Entä tilikausi?` -muodot) ratkaistaan deterministisesti Säännöt-sivun kohdasta 10 ennen payloadin rakentamista, joten se ei käytä Mistralia eikä `lue_sivu`-työkalukierrosta. Hallitus-, puheenjohtaja- ja yksiselitteisesti lähdesivulle osuvat henkilökysymykset käyttävät palvelimen varmentamaa lähdeotetta ja yhtä tavallista Mistral-kutsua. Muissa pakotetuissa kysymyksissä `tool_choice: "any"` hyväksytään vain, jos ensimmäinen vastaus sisältää ainakin yhden rakenteellisesti kelvollisen `lue_sivu`-kutsun ja positiivisen `sivu_id`:n; pakotettua plain-text-vastausta ei koskaan käsitellä lopullisena vastauksena.
...
12. **Tilikausilähde (#604), henkilölähde ja sivun lukutyökalu (#501)**: kysy `Mikä on tilikausi?`, tavallinen taivutusmuoto ja `Entä tilikausi?` → jokainen palauttaa Säännöt-sivun kohdan 10 tekstin ja permalinkin ilman Mistral- tai työkalukutsua. Kysy puhtaissa vuoroissa `Kuka on puheenjohtaja nyt?`, `Kuka on Antti Rytkönen?`, `Kuka on Mauri Rytkönen?`, `Kuka on Ilkka Rytkönen?`, `Kuka on Viljo Rytkönen?`, `Kuka on Teuvo Rönkkö?` ja `Kerro Teuvo Rönköstä.` → yksiselitteisen julkisen osuman pitää vastata lähdeotteen perusteella yhdellä API-kutsulla ilman sivunlukulaskurin kasvua. Moniselitteisen henkilön pitää jäädä pakotettuun fail-closed-työkalupolkuun. Aiheuta yksi turvallinen 502 ja kysy sen jälkeen `Miten liityn jäseneksi?` → jäsenyysvastaus ei saa vastata samalla epäonnistuneeseen henkilökysymykseen. Merkitse testisivu "Vain jäsenille" (#392) → chatti ei saa kertoa sen sisältöä millään kysymyksellä. Kysy pitkän, moniosaisen sivun toinen yksityiskohta (esim. nimenkirjoitusoikeus) → pakotetun ensimmäisen vastauksen pitää sisältää kelvollinen sivunlukukutsu, eikä plain-text-fallbackia hyväksytä.
...
Yksikkötestit (`tests/ChatProxyTest.php`) kattavat puhtaat helperit: rate limit -päätös, viestien valmistelu/katkaisu, rikkinäisen assistant-historian suodatus, vastauksen poiminta ja #604:n lopullisen vastauksen `finish_reason`-/sisältövalidointi sekä system-prompt ja prompt-välimuistikytkentä. `tests/ChatLiveContextTest.php` kattaa ajantasaisen tietolohkon (tapahtumat, jäsenyystuotteet, muut verkkokaupan tuotteet). `tests/ChatSitemapTest.php` kattaa sivustokarttalohkon (julkaistut sivut, arkistolinkit, rajaukset, kytkentä system-promptiin). `tests/ChatUsageStatsTest.php` kattaa käyttötilastojen laskurit, turvallisen usage-poiminnan, tallennuksen, yhteenvedon ja uudet staattiset virhetyypit (#472/#567/#604). `tests/ChatPageToolTest.php` kattaa sivun lukutyökalun, suoran tilikausilähteen ja palvelimen henkilölähdehaun: hallitus-/puheenjohtajaosumat, taivutettu sukunimi, moniselitteinen osuma sekä rajattujen sivujen fail-closed-käytös. `tests/ChatRequestHandlerTest.php` varmistaa REST-käsittelijästä asti yhden kutsun puheenjohtajapolun sekä sen, etteivät pakotettu plain text, virheellinen työkalukutsu tai `finish_reason: length` päädy käyttäjälle. Onnistunut elävä HTTP-/työkalusilmukka, selainhistorian rollback ja Dashboard-widget varmistetaan lisäksi yllä olevalla manuaalisella curl-/selaintestillä.
