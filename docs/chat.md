...
2. **Sivustokartta** (`rytkoset_theme_chat_get_sitemap_context()`): automaattisesti WordPressistä koottu lista sivuston julkaistuista sivuista (otsikko + permalink), julkaistuista tapahtumista (merkintä `(tapahtuma)`) sekä tapahtuma- ja albumiarkistoista. Tapahtumat listataan ennen sivuja, koska lohkon 6000 merkin raja katkaisee lopun: tapahtumia on vähän, ja ne ovat ainoa lähde ohjelma- ja aikataulukysymyksiin. Jos sivumäärä kasvaa niin, että lohko katkeaa, nosta `..._sitemap_max_length`-arvoa tai laske `..._sitemap_max_pages`-arvoa — tapahtumat säilyvät joka tapauksessa. Prompt kieltää viittaamasta muihin kuin lähteissä annettuihin osoitteisiin — tuotannossa havaittiin, että ilman sivukarttaa malli keksi osoitteen `/kuvat/`, jota ei ole olemassa. Uusi julkaistu sivu näkyy sivukartassa heti ilman koodimuutosta. Kun sivun lukutyökalu (#501) on käytössä, sivuriveillä on lisäksi `(sivu-id: N)` -merkintä ja julkisten sivujen otsikoista/erisnimistä koottu lyhyt `aiheita:`-hakuvihje, jolla malli voi valita oikean sivun `lue_sivu`-työkalulle. Hakuvihje ei ole faktavastauksen lähde.
...
Tapahtumarungot ovat mukana, koska tapahtuman ohjelma, aikataulu, tarjoilut ja käytännön ohjeet ovat vain tapahtuman omalla sivulla: live-konteksti antaa tapahtumasta vain rakenteisen tiivistelmän (päivä, kello, paikka, maksu, ilmoittautumisen takaraja), joten esimerkiksi `Mitä ohjelmaa juhlissa on?` tai `Milloin buffet on?` ei ollut aiemmin vastattavissa millään lähteellä. Albumit (`gallery_album`) ja blogikirjoitukset (`post`) eivät edelleenkään ole luettavia — ne näkyvät vain arkistolinkkeinä. Albumikuvaukset sisältävät elossa olevien henkilöiden nimiä ja rooleja, joten niiden avaaminen kolmannen osapuolen API:lle on erillinen päätös, joka vaatii `docs/tietosuoja.md`:n tarkistuksen.
...
**Vuotosuoja:** `lue_sivu`-työkalu palauttaa vain julkaistuja, salasanattomia `page`- ja `rytkoset_event`-sisältöjä (`rytkoset_theme_chat_get_page_tool_post_types()`), ja molemmat kulkevat saman `rytkoset_theme_chat_get_public_source_post()`-vartijan läpi kuin palvelimen lähde-esihaku: sivuille täysi jäsensivutarkistus, tapahtumille julkaisu- ja salasanatarkistus. Muut sisältötyypit, kuten albumit ja blogikirjoitukset, eivät kelpaa. Jäsenille rajatut sivut (#392, `_rytkoset_members_only`) suodatetaan **ehdottomasti ja katsojasta riippumatta**. Kaikki sivun epäämissyyt käyttävät samaa geneeristä käsittelyä, eikä lähteetön fallback kerro, onko rajattua sisältöä olemassa.
