...
- Pakotettu `tool_choice: "any"` noudattaa [Mistralin function calling -ohjetta](https://docs.mistral.ai/studio-api/conversations/function-calling): ensimmäisellä kierroksella hyväksytään vain kelvollinen `lue_sivu`-kutsu, jolla on positiivinen `sivu_id`. Plain-text-vastaus, väärä työkalu tai virheelliset argumentit hylätään — mallin varmentamatonta tekstiä ei koskaan näytetä kävijälle.
- **Havaittu latenssiriski (paikallinen mittaus 22.7.2026):** `tool_choice: "any"` on `mistral-medium-latest`-mallilla huomattavasti hitaampi kuin tavallinen kutsu. Sama aihe kysyttynä ilman pakotusta (`Kerro kaupan maksamisesta.`) vastasi 3,5 s ja 8,2 s, kun pakotuksen laukaiseva muotoilu (`Mitä maksutapoja on olemassa?`) ylitti 20 sekunnin `wp_remote_post`-timeoutin molemmilla yrityksillä ja päätyi turvalliseen 502:een (virhetyyppi `network`, työkalukutsulaskuri ei kasvanut eli ensimmäinen kutsu ei ehtinyt palata). Perusviive samassa ympäristössä oli 2,2–4,5 s. Ilmiö ei johdu hyötykuorman koosta: se toistui myös 1407 merkin otteella. Jos sama näkyy devissä tai tuotannossa, oikea korjaus ei ole timeoutin nosto (kokeiltu 30 s, kutsut kestivät silti 17,5–35,1 s) vaan pakotetun kierroksen välttäminen — esimerkiksi laajentamalla palvelinpuolen esihaku näihin käsitekysymyksiin, jolloin vastaus syntyy yhdellä `tool_choice: "none"` -kutsulla kuten hallituskysymyksissä (4,5 s).
- Hylkäys **ei kuitenkaan enää päätä pyyntöä tekniseen virheeseen**: pakotus pudotetaan ja sama pyyntö lähetetään kerran uudelleen tavallisella automaattisella työkaluvalinnalla. Aiemmin fail-closed muutti kelvollisen kysymyksen kävijälle umpikujaksi — tuotannossa `Missä maissa tietojani käsitellään?` päätyi virheilmoitukseen, vaikka vastaus lukee tietosuojasivulla. Nimetyt faktakysymykset eivät kulje tätä polkua lainkaan: ne on jo siinä vaiheessa vastattu varmennetusta lähteestä tai turvallisella lähteettömällä 200-vastauksella, joten uusintayritys ei avaa reittiä muistista vastaamiseen. Uusinta tehdään enintään kerran, ja #604:n vastausvalidointi pätee siihen samalla tavalla. Virhetilaston `forced_tool_missing` kirjataan silti, joten tilanne pysyy näkyvissä Dashboard-vimpaimessa vaikka vastaus onnistuisi.
...
**Kulusuojat:** työkalukierroksia enintään 2 per käyttäjäviesti oletuksena (suodatin `rytkoset_theme_chat_page_tool_max_rounds`, kova yläraja 3), sivusisältö rajataan 5000 merkkiin (`..._page_tool_max_length`), sivunluku suoritetaan enintään kolmelle työkalukutsulle per kierros ja jokainen suoritettu kutsu kirjataan käyttötilastoihin (#472). Rate limit kuluu kerran per käyttäjäviesti — sisäiset jatkokutsut eivät kierrä sitä. Huomaa, että työkalullinen kysymys kestää yleensä kaksi tai kolme API-kierrosta ja maksaa enemmän tokeneita.

**Pitkän sivun ote:** merkkiraja on edelleen 5000, mutta se käytetään eri tavalla. Aiemmin ylimenevä osa katkaistiin paljaalla `mb_substr()`-kutsulla sivun alusta ilman mitään merkintää. Sivuston keskeisimmät viitesivut ovat selvästi pidempiä — tietosuojaseloste noin 17 251 ja maksu- ja toimitusehdot noin 9 838 merkkiä — joten vastaus jäi säännönmukaisesti katkaisun taakse eikä malli tiennyt lukeneensa vain osan. Tuotannossa tämä näkyi kahdella tavalla: uutiskirjeen peruutus ohjattiin sähköpostiin, vaikka sivu kertoo peruutuslinkistä jokaisen uutiskirjeen alalaidassa (merkki 7148), ja kysymys `Missä maissa tietojani käsitellään?` ei saanut sivun omaa siirtotietoa lainkaan.

Nyt rajan ylittävä sivu palautetaan **kysymykseen pisteytettynä otteena**: `rytkoset_theme_chat_get_page_query_terms()` poimii käyttäjän viestistä vähintään viisikirjaimiset sisältösanat (kysymys- ja täytesanat pudotetaan), ja `rytkoset_theme_chat_get_scored_page_excerpt()` pisteyttää rivit sen mukaan, kuinka moneen eri kysymystermiin ne osuvat (tasatilanteessa osumien määrä ratkaisee), valitsee parhaat rivit lähikonteksteineen budjetin täyttymiseen asti ja palauttaa ne dokumenttijärjestyksessä. Jos kysymyksestä ei irtoa termejä tai mikään ei osu, palataan entiseen alkukatkaisuun. Ote merkitään aina loppuun lisättävällä huomautuksella (`rytkoset_theme_chat_get_page_tool_excerpt_notice()`), joka kertoo mallille ettei se saanut koko sivua.

Pisteytys on olennainen, ei kosmeettinen: pelkkä dokumenttijärjestyksessä valitseminen täyttää budjetin sivun alusta aina kun kysymystermi on sivulla yleinen. Tietosuojaselosteella `tieto*` osuu 24 %:iin ja `käsit*` 21 %:iin kaikista riveistä, joten sivun loppupuolen siirto-osio ei mahtunut mukaan edes 6000 merkillä. Pisteytettynä sama vastaus löytyy jo noin 3000 merkillä, ja mitatut otteet ovat 1407–2688 merkkiä eli selvästi vanhaa 5000 merkin alkukatkaisua pienempiä. Merkkirajan nostoa 8000:een kokeiltiin ensin, mutta se hylättiin: pisteytys ratkaisee saman ongelman pienemmällä hyötykuormalla.
...
12. **Tilikausilähde (#604), nimetty lähdehaku ja sivun lukutyökalu (#501)**: varmista tilikausipolku ja aiemmat henkilötestit. Kysy lisäksi `Voinko ostaa kirjan Rytkösiä sukupolvesta toiseen?` → vastauksen pitää käyttää Sukututkimus-sivun loppuunmyyty-tietoa. `Milloin Tampereen sukukokous pidettiin?` saa käyttää tulevan tapahtuman ja kuljetuksen sivuja mutta ei hallituksen paikkarivejä. `Milloin Runnin sukukokous pidettiin?`, `Milloin Helsingin sukukokous pidetään?`, `Milloin Rovaniemellä on pidetty sukukokous?` ja `Kuka on Ruotsin presidentti?` palauttavat nykyisellä sisällöllä turvallisen lähteettömän HTTP 200 -vastauksen ilman Mistral- tai sivunlukukutsua. Lisää testisivulle paikka ja sukukokous eri kappaleisiin → se ei saa kelvata osumaksi. Merkitse sivu vain jäsenille tai tapahtuma luonnokseksi/salasanalliseksi → sisältö ei saa päätyä lähdeotteeseen. Pronominikysymyksen kelvoton pakotettu työkaluvastaus ei enää palauta 502:ta vaan yrittää kerran uudelleen ilman pakotusta; mallin varmentamaton ensivastaus ei saa näkyä kävijälle. Kysy lisäksi `Missä maissa tietojani käsitellään?` → tulee tietosuojasivun oma tieto (EU-alue / Mistral AI SAS), ei virheilmoitusta eikä pelkkää sähköpostiohjausta. Kysy uutiskirjeen tilaus ja peruutus useassa muodossa (`Miten tilaan uutiskirjeen?`, `Miten perun uutiskirjeen tilaamisen?`, `Voinko peruuttaa uutiskirjeen tilaamisen?`) → jokaiseen pitää tulla nopea vastaus pysyvästä kontekstista (alalaidan lomake, Oma tili → Uutiskirje, peruutuslinkki uutiskirjeen alalaidassa) **ilman 502-virhettä** — nämä eivät saa ohjautua pakotetulle sivunluvulle. Kysy `Mitä maksutapoja on olemassa?` → vastauksessa saa mainita vain Paytrailin ja sen, että tavat näkyvät kassalla; yksittäisiä maksutapoja (pankki, mobiili, kortti, lasku, osamaksu) ei saa luetella. Kysy hallituskokoonpanoa vielä muodoissa `Ketkä kuuluvat hallitukseen?`, `Luettele hallituksen jäsenet.` ja `Näytä hallituksen kokoonpano.` → kaikki antavat saman hallitussivun listan. Kysy `Voiko kuka tahansa liittyä sukuseuran jäseneksi?` ja `Saako sukukirjaa kirjastosta?` → kumpikaan ei saa palauttaa lähteetöntä vakiovastausta.
