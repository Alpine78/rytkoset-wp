...
> **Sisäinen luonnos — tarkistettava ennen käyttöä.** Tätä asiakirjaa ei julkaista WordPressin tietosuojasivulla. Asiakirja on laadittu reposta todennettavien tietovirtojen perusteella 15.7.2026 ja varmuuskopiointisuunnitelma on lisätty 13.8.2026 (#421). Tuotantoympäristön asetukset, sopimukset, kaikki vastaanottajat ja tosiasialliset säilytysajat on vahvistettava vastuuhenkilön kanssa.

> **Turnstilen tila 23.7.2026 klo 19.09:** bottisuoja oli aktiivinen tuotannon rekisteröintilomakkeessa ja rajattu pois kirjautumisesta sekä salasanan palautuksesta. Julkisessa savutestissä ei havaittu `cf_clearance`-evästettä. Managed-tila, hostname-rajaus ja pre-clearance pois vahvistetaan vielä Cloudflare-hallinnasta.
...
| --- | --- | --- | --- | --- | --- | --- |
| Käyttäjätilit ja jäsenedut | Tilin ja jäsenetujen toteuttaminen; sopimus / rekisteröidyn pyytämä palvelu ja oikeutettu etu | Käyttäjät ja jäsenet: nimi, sähköposti, profiili- ja kirjautumistiedot, jäsenyyden tyyppi ja voimassaolo, käsittelyloki | Rekisteröity; jäsenrekisteri | Hosting-palvelu; yhdistyksen valtuutetut ylläpitäjät; Gravatar / Automattic käsittelee kirjautuneen käyttäjän sähköpostiosoitteesta muodostettua tiivistettä profiilikuvan tarkistamista varten ja voi käsitellä tietoja EU/ETA:n ulkopuolella | Aktiivisen tilin ajan; odottavat jäsenyystiedot vain linkitystä varten; lokin säilytysaika on vahvistettava | Käyttöoikeudet, salasanojen tiivistys, HTTPS ja [`docs/tietoturva.md`](tietoturva.md):ssä kuvatut teemakovennukset; palvelintason tila vahvistettava |
| Avoimen käyttäjärekisteröinnin bottisuoja | Rekisteröinnin suojaaminen automatisoidulta väärinkäytöltä; oikeutettu etu ja tietoturvavelvoitteet | Rekisteröityjät: IP-osoite, TLS-sormenjälki, User-Agent, Turnstilen sitekey ja alkuperä sekä kertakäyttöinen varmennustunniste | Rekisteröidyn laite ja rekisteröintipyyntö | Cloudflare, Inc. toimii käsittelijänä sivuston suojaamisessa ja itsenäisenä rekisterinpitäjänä botintunnistuksen kehittämisessä; maailmanlaajuinen käsittely ja mahdolliset EU/ETA-alueen ulkopuoliset siirrot Cloudflaren ehtojen mukaan | Turnstile-tunniste vanhenee viidessä minuutissa ja on kertakäyttöinen. Cloudflaren signaalien säilytysaika sekä DPA ja siirtoperuste on vahvistettava. Lisäosan debug-loki pidetään pois käytöstä; päällä ollessaan se säilyttäisi WordPressissä enintään 50 tarkistuksen ajan, tuloksen, virhekoodin, IP-osoitteen ja sivupolun ilman aikaperusteista poistoa | Managed-widget; hostname-rajaus; palvelinpuolen Siteverify; pre-clearance pois; lähetyspainike lukittu varmennukseen; fail-closed; debug-loki pois; teeman honeypot ja domain-estot lisäkerroksena |
...
| Palvelinlokit, varmuuskopiot ja tietoturva | Palvelun turvallisuus, häiriöselvitys ja palautettavuus; oikeutettu etu ja tietoturvavelvoitteet | Sivuston käyttäjät: IP-osoite, pyyntö- ja virhetiedot; varmuuskopioissa kaikki edellä kuvatut tuotantotiedot ja media | Sivuston käyttö ja palvelin | Domainhotelli; Backblaze, Inc. B2 Cloud Storage -käsittelijänä, tallennus EU Central / Amsterdam; mahdollinen ylläpitokumppani Klik, jos sillä on pääsy palvelimeen tai varmuuskopioihin **[Backblaze-käyttöönotto, DPA, alikäsittelijät sekä Klikin rooli ja pääsy vahvistettava]** | Päivätyt DB- ja `wp-content`-arkistot 30 vrk; mediamirrorin nykyversio lähdetiedoston elinkaaren ajan ja poistettu/korvattu versio 30 vrk; lokien säilytysaika vahvistettava. Tuotannosta poistettu tieto poistuu varmistuskierrossa, ja vanhan kopion palautuksessa poistot/anonymisoinnit toteutetaan uudelleen | TLS siirrossa, `rclone crypt` -asiakassalaus sisällölle ja nimille, yksityinen bucket, bucket-kohtainen avain, 2FA, salausavaimet erillään B2-tilistä, työtila ja lokit `public_html`:n ulkopuolella, rajatut vastuut sekä dokumentoitu palautustesti; toteutunut tila vahvistettava |
...
- Backblaze, Inc.: B2-off-site-varmistukset; EU Central -tallennusalue, DPA osana käyttöehtoja, alikäsittelijät, kansainvälisten siirtojen suojatoimet, 2FA ja käyttöoikeudet vahvistetaan käyttöönotossa
- Mistral AI SAS: tukichatin viestit; käsittelysopimus, käsittelyalue ja alikäsittelijät
- Gravatar / Automattic: kirjautuneiden käyttäjien profiilikuvat; tietosuojarooli, EU/ETA-alueen ulkopuolisten siirtojen suojatoimet ja palvelun käytöstä poistamisen vaikutus
- Cloudflare, Inc.: Turnstilen tekniset signaalit; käsittelijärooli sivuston suojaamisessa, itsenäinen rekisterinpitäjä botintunnistuksen kehittämisessä, DPA, alikäsittelijät, säilytys ja EU/ETA-alueen ulkopuolisten siirtojen suojatoimet
